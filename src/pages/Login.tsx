...
import { login, authStorage } from '../services/auth'
    ...
    <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-start">
        ...
        <div className="bg-white/70 supports-backdrop-filter:bg-white/60 backdrop-blur-xl rounded-2xl border border-white/60 shadow-[0_12px_40px_rgba(2,6,23,0.12)] p-6 sm:p-8">
          ...
          <form className="grid gap-4" aria-label="Sign in form" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-slate-700 mb-2">Email</label>
              <input id="email" type="email" required placeholder="you@startup.com" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-200" />
            </div>
            <div>
              ...
              <input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-200" />
            </div>
            <Button type="submit" variant="primary" size="md" className="mt-2 w-full" disabled={isSubmitting}>{isSubmitting ? 'Signing In…' : 'Sign In'}</Button>
          </form>
